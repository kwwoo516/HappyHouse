...
public class BoardReplyController {
	...
	public ResponseEntity<String> writeReply(@RequestBody @ApiParam(value = "댓글 정보.", required = true) ReplyDto replyDto) throws Exception {
		...
		if (boardreplyService.writeReply(replyDto)) {
	...
	public ResponseEntity<List<ReplyDto>> getReply(@PathVariable("articleno") @ApiParam(value = "댓글을 얻어올 글번호.", required = true) int articleno) throws Exception {
		...
		return new ResponseEntity<List<ReplyDto>>(boardreplyService.getReply(articleno), HttpStatus.OK);
	}
	...
	public ResponseEntity<String> modifyReply(@RequestBody @ApiParam(value = "수정할 댓글 정보.", required = true) ReplyDto replyDto) throws Exception {
		...
		if (boardreplyService.modifyReply(replyDto)) {
			return new ResponseEntity<String>(SUCCESS, HttpStatus.OK);
		}
	...
	public ResponseEntity<String> deleteReply(@PathVariable("replyno") @ApiParam(value = "삭제할 댓글 번호.", required = true) int replyno) throws Exception {
		...
		if (boardreplyService.deleteReply(replyno)) {
			return new ResponseEntity<String>(SUCCESS, HttpStatus.OK);
		}
		return new ResponseEntity<String>(FAIL, HttpStatus.NO_CONTENT);
	}
