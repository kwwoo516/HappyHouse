...
public class NoticeController {
	...
	@GetMapping
	public ResponseEntity<List<BoardDto>> listArticle(@ApiParam(value = "공지사항을 얻기위한 부가정보.", required = true) BoardParameterDto boardParameterDto) throws Exception {
		logger.info("listArticle - 호출");
		return new ResponseEntity<List<BoardDto>>(noticeService.listArticle(boardParameterDto), HttpStatus.OK);
	}
	
	@ApiOperation(value = "공지사항 글보기", notes = "글번호에 해당하는 공지사항의 정보를 반환한다.", response = BoardDto.class)
	@GetMapping("/{articleno}")
	public ResponseEntity<BoardDto> getArticle(@PathVariable("articleno") @ApiParam(value = "얻어올 공지사항의 글번호.", required = true) int articleno) throws Exception {
	...
	@PutMapping
	public ResponseEntity<String> modifyArticle(@RequestBody @ApiParam(value = "수정할 공지사항 정보.", required = true) BoardDto boardDto) throws Exception {
	...
	public ResponseEntity<String> deleteArticle(@PathVariable("articleno") @ApiParam(value = "삭제할 공지사항의 글번호.", required = true) int articleno) throws Exception {
